  ...
  socket.on("message", async (msg) => {
    try {
      ...
      if (parsedMsg.type == "check-admin") {
        ...
        const isAdmin =
          roomAdmins.get(parsedMsg.payload.roomId) === parsedMsg.payload.email;
      ...
      if (parsedMsg.type == "upvote") {
        ...
        try {
          ...
          if (sockets && sockets.size > 0) {
            ...
            console.log(
              `No sockets found for room ${parsedMsg.payload.roomId}`
            );
          }
      ...
      if (parsedMsg.type == "close") {
        try {
          ...
          if (roomSockets && roomSockets.size > 0) {
            console.log(
              `Notifying ${roomSockets.size} clients that room ${
                parsedMsg.payload.roomId
              } is closing`
            );
            const closeMessage = {
              type: "room-closed",
